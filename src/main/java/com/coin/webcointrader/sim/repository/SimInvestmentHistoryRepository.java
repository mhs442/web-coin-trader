 ...
 */
@Repository
public interface SimInvestmentHistoryRepository extends JpaRepository<SimInvestmentHistory, Long> {

    /**
     * 사용자의 모의 투자 히스토리를 날짜 범위 + 동적 정렬로 조회한다.
     *
     * @param userId 사용자 ID
     * @param start  조회 시작일시
     * @param end    조회 종료일시
     * @param sort   정렬 조건
     * @return 모의 투자 히스토리 목록
     */
    List<SimInvestmentHistory> findByUserIdAndCreatedAtBetween(
            Long userId, LocalDateTime start, LocalDateTime end, Sort sort);

    /**
     * 사용자의 모의 투자 히스토리를 날짜 범위 + 페이징 조회한다.
     *
     * @param userId   사용자 ID
     * @param start    조회 시작일시
     * @param end      조회 종료일시
     * @param pageable 페이징 조건
     * @return 모의 투자 히스토리 페이지
     */
    Page<SimInvestmentHistory> findByUserIdAndCreatedAtBetween(
            Long userId, LocalDateTime start, LocalDateTime end, Pageable pageable);

    /**
     * 사용자의 모의 투자 히스토리 손익 합산을 조회한다. (날짜 범위 조건)
     * 결과: [이익금 합산, 손해금 합산]
     *
     * @param userId 사용자 ID
     * @param start  조회 시작일시
    ...
    /**
     * 사용자의 모의 투자 히스토리 손익 합산을 조회한다. (날짜 범위 + 심볼 조건)
     * 결과: [이익금 합산, 손해금 합산]
     *
     * @param userId 사용자 ID
     * @param start  조회 시작일시
    ...
    /**
     * 사용자의 모의 투자 히스토리를 심볼 포함 + 날짜 범위 + 페이징 조회한다. (DB 레벨 심볼 필터)
     *
     * @param userId   사용자 ID
     ...
     * @return 모의 투자 히스토리 페이지
     */
    Page<SimInvestmentHistory> findByUserIdAndSymbolContainingIgnoreCaseAndCreatedAtBetween(
    ...
    /**
     * 선택한 ID 목록 중 해당 사용자 소유 모의 투자 히스토리를 삭제한다. (선택 삭제)
     *
     * @param ids    삭제할 모의 투자 히스토리 ID 목록
     * @param userId 사용자 ID (권한 검증)
     */
    void deleteAllByIdInAndUserId(List<Long> ids, Long userId);

    /**
     * 해당 사용자의 전체 모의 투자 히스토리를 삭제한다. (전체 삭제)
     *
     * @param userId 사용자 ID
     */
    ...
    /**
     * 모의 투자 ID와 사용자 ID로 단건 조회 (권한 검증 포함).
     *
     * @param id     모의 투자 히스토리 ID
     * @param userId 사용자 ID
     * @return 모의 투자 히스토리 (없으면 empty)
     */
    java.util.Optional<SimInvestmentHistory> findByIdAndUserId(Long id, Long userId);
}
